...
const PostsList = () => {
  ...
  const handleSort = (e) => {
    ...
    if (sort === "title" || sort === "body") {
      setFilteredPosts([...posts].sort((a, b) => a[sort].localeCompare(b[sort])))
    }else if (sort === "date") {
      setFilteredPosts([...posts].sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)))
    }else {
      setFilteredPosts([...posts].sort((a, b) => a.id - b.id))
    }
  }
    ...
    <div className="usersList">
      ...
      {(id && requestName === "id" && <GetPostById id={id} onGetId={onGet} fetchPosts={fetchPosts} />) ||
        (filteredPosts.length > 0
          ...
          : posts.map((post, index) => (<PostItem key={index} post={post} fetchPosts={fetchPosts} />))
        )}
